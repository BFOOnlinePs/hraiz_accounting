...
//        $user_notes = UserNotesModel::get();
            ...
            //     $query->select('*')->from(with(new UserLevels())->getTable())
            //     ->where('user_id',auth()->user()->id)->whereIn('role_id',[1,2]);
            // })->first();
            if(in_array(['1','2'],json_decode(auth()->user()->user_role))){
        ...
        if (in_array(['1','3'],json_decode(auth()->user()->user_role))) {
//            $user_notes = UserNotesModel::get();
