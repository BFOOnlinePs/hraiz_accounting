...
class DeliveryCompanyController extends Controller
{
    ...
    {
        $data = User::whereJsonContains('user_role', ['5'])->get();
        return view('admin.users.delivery_company.index', ['data' => $data]);
    }

    public function add()
    {
        return view('admin.users.delivery_company.add');
    }
    ...
    public function create(Request $request)
    {
        ...
        if ($data->save()) {
            return redirect()->route('users.delivery_company.index')->with(['success' => 'تم اضافة البيانات بنجاح']);
        } else {
    ...
    public function edit($id)
    {
        ...
        return view('admin.users.delivery_company.edit', ['data' => $data,'user_role'=>$user_role]);
    }
    ...
    public function update($id, Request $request)
    {
        ...
        $data->user_iban_number = $request->user_iban_number;
        if ($data->save()) {
            return redirect()->route('users.delivery_company.edit', ['id', $id])->with(['success' => 'تم اضافة البيانات بنجاح']);
        } else {
            return redirect()->back()->withInput();
        }
...
//        }
        return view('admin.users.delivery_company.details', ['data' => $data, 'company_contact_person' => $company_contact_person]);
    }
}
