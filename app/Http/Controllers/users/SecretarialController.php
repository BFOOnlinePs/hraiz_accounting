...
class SecretarialController extends Controller
{
    public function index(){
        $data = User::whereJsonContains('user_role',['3'])->get();
        return view('admin.users.secretarial.index',['data'=>$data]);
    }

    public function add(){
        return view('admin.users.secretarial.add');
    }
...
//        $data->user_iban_number = $request->user_iban_number;
        if ($data->save()){
            return redirect()->route('users.secretarial.index')->with(['success'=>'تم اضافة البيانات بنجاح']);
        }
        else{
    ...
    public function edit($id){
        ...
        return view('admin.users.secretarial.edit',['data'=>$data,'user_role'=>$user_role]);
    }
...
//        $data->user_iban_number = $request->user_iban_number;
        if ($data->save()) {
            return redirect()->route('users.secretarial.edit',['id'=>$id])->with(['success' => 'تم تعديل البيانات بنجاح']);
        } else {
            return redirect()->back()->withInput();
        }
    }
...
//        }
        return view('admin.users.secretarial.details',['data'=>$data,'company_contact_person'=>$company_contact_person]);
    }
}
