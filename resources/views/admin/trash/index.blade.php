...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-header">
            <h3 class="text-center">الطلبيات المحذوفة</h3>
        </div>
                        ...
                        <tr class="">
                            ...
                            <td>
                                @if($key->order_status == -1)
                                    <span>محذوفة</span>
                                @else
                                    <span>{{ \App\Models\OrderStatusModel::where('id',$key->order_status)->value('name') }}</span>
                                @endif
                            </td>

                            <td>
                                <a href="{{ route('procurement_officer.orders.product.index',['order_id'=>$key->id]) }}"
                                   class="btn btn-dark btn-sm">عرض الطلبية</a>
                                <button type="button" onclick="getReferenceNumber({{ $key->id }})"
                                        class="btn btn-success btn-sm" data-toggle="modal"
                                        data-target="#modal-reference_number">
                                    تعديل الرقم المرجعي
                                </button>
                                <a href="{{ route('trash.updateOrderStatus',['id'=>$key->id]) }}" class="btn btn-primary btn-sm"><span class="fa fa-backward"></span></a>
{{--                                <a href="{{ route('orders.procurement_officer.delete_order',['id'=>$key->id]) }}"--}}
